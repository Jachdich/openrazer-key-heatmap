HEATMAP_LOCATION = "/home/USERNAME/.heatmap.json" #EDIT THIS, VERY VERY IMPORTANT
SARCASM_MODE     = None #Change to path of sarcasm mode file to enable sarcasm mode.
#This reads from the file and if it is 'true' it toggles caps lock every keypress.
        ...
        * Pressing any key will increment a statistical number in a dictionary used for generating
        ...
        try:
            ...
            if key_press == 'release':
                ...
                with open(HEATMAP_LOCATION, "w") as f:
                    f.write(json.dumps(heatmap))

                if not SARCASM_MODE == None:
                    if not key_name in FORBIDDEN_KEYS and open(SARCASM_MODE, "r").read().replace("\n", "") == "true":
                        subprocess.run("xdotool key Caps_Lock".split(" "))
                ...
                # Not recording anything so if a macro key is pressed then run
                    ...
                    #with open("/home/hazel/MACRO_THINg.log", "a") as f:
                    #    f.write("GOT MAYBE MACRO: " + key_name + ", AND THIS: " + ",".join([str(i) for i in self._macros]))
